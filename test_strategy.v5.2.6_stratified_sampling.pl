  ################################
#
#  5.2.4 updated as of 19 JUN 2020
...
sub new_infection{
	my @new_args = @_; # established an array which contains parameters for subroutine(function)
	my $ninf = $new_args[0];
	my $infect_time_s = $new_args[1];
	my $pos_infect = 0;
	if(exists($h_immune{$ninf}) || exists($infect_time{$ninf})){} #if a person is immune or already infected, do nothing
	else{
		push(@infected, $ninf); #create an infected individual
		$infect_time{$ninf} = $infect_time_s; #assign them an infection time
		$sympto{$ninf} = bern($sympto_rate); #randomly determine symptomatic vs asymptomatic
		$active_infection{$ninf} = 1; #establishes activley infected for that person
		$pos_infect = 1; #ID for positivley infected (returns empty if individual is not infected)
		$total_infections++; # adds to total infection rate of population
...
sub count_group_inc {
	...
	if (exists($h_team{$ninf})){ #if they are on a team
		my $team = $h_team{$ninf};
		$team_infect[$team]++; #infects the whole team
	}
	my $company = $h_company{$ninf}; #defines company
	$company_infect[$company]++; #infects company
	foreach my $section (@{$section_sched{$ninf}}){ #for each class section
		$n_section{$section} = $n_section{$section} + 1;
		$section_infect[$section]++; #infect whole section?
	}
}
...
sub count_group_dec {
	...
	if (exists($h_team{$ninf})){
		my $team = $h_team{$ninf};
		$team_infect[$team]--; ##disinfect team
	}
	my $company = $h_company{$ninf};
	$company_infect[$company]--;##disinfect company
	foreach my $section (@{$section_sched{$ninf}}){
		$n_section{$section} = $n_section{$section} + 1;
		$section_infect[$section]--; ##disinfect company
	}
}


#testing function; increments TP/FN/TN/FP counts [True Positive/False Negative ...]
sub test_cadet{
	my $cdt = $_[0];
	my $test_result = 0;
	if($active_infection{$cdt} == 1){
		...
		else{$FN_count++;} #randomly infects a cadet and then assigns a test result randomly
	}
	else{
		...
		if($test_result == 0){$TN_count++;}
			...
			push(@quarantine_time,$i);  #removing if false positive?
		}
	}
	$tested++;
	$ind_tested{$cdt} = 1;
...
###############################
############################### Critical parameters: (Initating parameters)

$test_qty = $ARGV[0]; #specify random number to test
...
$asympto_start = 0; # nbr of asymptomatic to start (880 for ~20%)
...
##############################
##############################
for($k=0;$k<1;$k++){ #itnitial parameters
	$total_infections = $IT;
	$TP_count = 0;
	$TN_count = 0;
	$FP_count = 0;
	...
	$initial_infection_count++;
	@a_immune = @all_cadets[$initial_infection_count..$asympto_start];
